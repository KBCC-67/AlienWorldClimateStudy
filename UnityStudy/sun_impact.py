...
for day in range(days_of_year):
    ...
    for y in range(mapshape[0]):
        ...
        if day%7==1:
            impact=0#假设联合星所围绕的olympus和木星质量相仿，联合星运转轨道半径和木卫三相似，那么每七天被遮挡一次
        sun_heat_impact_matrix[day][y]=impact
np.save("initial\\\\real_sun_heat_impact_matrix",sun_heat_impact_matrix)
print("day 0:")
...
plt.show()
print("day 274:")
plt.plot(np.arange(1024),sun_heat_impact_matrix[274])
plt.show()
