...
sun_heat_impact_matrix=np.load("initial\\\\real_sun_heat_impact_matrix.npy")#现在还没有，比较复杂，单独写个
...
days=365
global_mean_temperature_list=np.zeros((days))
global_mean_rain_list=np.zeros((days))
for i in range(days):
    print("day ",i)
...
#上面这个ret好像是判断操作是否成功，没有的话这个matrix就是个二元素列表。。。第一个元素是1，第二个元素才是咱要的矩阵
    ...
    #我们研究一下气体巨星对气候带来的7天一次的周期性变化问题：
    global_mean_temperature=temperature_matrix.mean()
    global_mean_temperature_list[i]=global_mean_temperature
    global_mean_rain=rain_cause_humidity_change.mean()
    global_mean_rain_list[i]=global_mean_rain
...
np.save("result\\\\temperature_list",global_mean_temperature_list)
np.save("result\\\\rain_list",global_mean_rain_list)